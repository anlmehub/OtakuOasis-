...
if (hasMongo) {
  ...
  catch { console.warn("Install mongoose with npm i mongoose"); }
}
...
async function initData() {
  ...
  if (usingMemory && DEMO_AUTO_DELETE_MINUTES > 0) {
    setInterval(() => { memoryStore.anime = []; console.log("🧹 In-memory demo auto-cleared."); }, DEMO_AUTO_DELETE_MINUTES * 60 * 1000);
  }
}
initData();
...
/* -----------------------------
   API Routes
------------------------------*/
app.get("/api/health", (req, res) => res.json({ ok: true, ...healthSnapshot() }));

// GET anime with pagination & search
app.get("/api/anime", async (req, res) => {
  try {
    ...
    let items = usingMemory ? memoryStore.anime : await AnimeModel.find().sort({ createdAt: -1 });
    if (search) {
    ...
    res.json({ page, limit, total: items.length, data: items.slice(start, start + limit) });
  } catch { res.status(500).json({ error: "Failed to fetch anime." }); }
});

// POST anime (optionally owner-protected)
app.post("/api/anime", async (req, res) => {
  try {
    if (REQUIRE_OWNER_FOR_WRITES && !isOwner(req)) return res.status(403).json({ error: "Forbidden" });
    ...
    if (usingMemory) {
      ...
      memoryStore.anime.unshift(item); return res.json(item);
    }
    const created = await AnimeModel.create({ title, description, episodes, status, streamUrl, downloadUrl });
    res.json(created);
  } catch { res.status(500).json({ error: "Failed to create anime." }); }
});
...
app.delete("/owner/demo", (req, res) => {
  ...
  res.json({ ok: false, message: "Mongo demo purge not implemented." });
});
